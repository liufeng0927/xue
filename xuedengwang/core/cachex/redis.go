...
func NewRedisCache(client *redis.Client, keyPrefix string, encoding encodingx.Encoding, newObject func() interface{}) Cache {
	return &redisCache{
		client:    client,
		KeyPrefix: keyPrefix,
		encoding:  encoding,
...
func (c *redisCache) Set(ctx context.Context, key string, val interface{}, expiration time.Duration) error {
	buf, err := encodingx.Marshal(c.encoding, val)
	if err != nil {
...
func (c *redisCache) Get(ctx context.Context, key string, val interface{}) error {
	...
	err = encodingx.Unmarshal(c.encoding, bytes, val)
	if err != nil {
...
func (c *redisCache) MultiSet(ctx context.Context, valueMap map[string]interface{}, expiration time.Duration) error {
	...
	for key, value := range valueMap {
		buf, err := encodingx.Marshal(c.encoding, value)
		if err != nil {
			logx.Errorf("marshal data err: %+v, value is %+v", err, value)
			continue
		}
		cacheKey, err := BuildCacheKey(c.KeyPrefix, key)
		if err != nil {
			logx.Errorf("build cache key err: %+v, key is %+v", err, key)
			continue
		}
	...
	for i := 0; i < len(paris); i = i + 2 {
		...
		default:
			logx.Errorf("redis expire is unsupported key type: %+v", reflect.TypeOf(paris[i]))
		}
	}
...
func (c *redisCache) MultiGet(ctx context.Context, keys []string, value interface{}) error {
	...
	for i, value := range values {
		...
		object := c.newObject()
		err = encodingx.Unmarshal(c.encoding, []byte(value.(string)), object)
		if err != nil {
			logx.Errorf("unmarshal data error: %+v, key=%s, cacheKey=%s type=%v", err,
