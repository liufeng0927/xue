  ...
  <el-card class="box-card">
      ...
      <div class="card-header">
        ...
        <div class="card-search">
          <el-row :gutter="12">
            ...
            <el-col :span="6">
              <el-button plain color="#2fa7b9" @click="searchS">搜索</el-button>
            ...
            <el-col :span="3">
              <el-input v-model="stuno" @keyup.enter.native="search"
                        placeholder="学号搜索（回车）"/>
            </el-col>
            <el-col :span="3">
              <el-input v-model="name" @keyup.enter.native="search"
                        placeholder="姓名搜索（回车）"/>
            </el-col>
            <el-col :span="2"
    ...
    <div class="table-box">
      <el-table row-key="id" element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column width="98px" label="学号" prop="stuno">
        ...
        <el-table-column prop="gradeClass" label="班级">
          <template #default="scope">
            <span>{{ scope.row.gradeClass }}</span>
          </template>
        </el-table-column>

        <el-table-column prop="course" label="学科">
          <template #default="scope">
          ...
          <template #default="scope">
            <div class="edit-score" v-if="scope.row.edit">
              ...
              <el-button type="success" size="small" @click="editScores(scope.row)"
                         style="margin: 0 0 10px 10px;">保存
              </el-button>
              <el-button type="warning" size="small" @click="cancel(scope.row)"
                         style="margin: 0 0 10px 10px;">取消
              </el-button>
            </div>
            <span v-else>
              {{ scope.row.score }}
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-popconfirm confirm-button-text="确定" cancel-button-text="取消"
                           icon-color="#626AEF" :title="'确定删除学生名为“'+scope.row.name+'”的成绩吗？'"
...
async function gradeClassList() {
  try {
    ...
    if (data.status === 200) {
      gradeClassOptions.value = data.result.gradeClassAll
    }
  } catch (e) {
    console.log(e)
  }
...
async function getAllCourseList() {
  try {
    ...
    if (data.status === 200) {
      courseOptions.value = data.result.courseAll
    }
  } catch (e) {
    console.log(e)
  }
...
const loadData = async (state: any) => {
  ...
  state.tableData = data.result.content
...
const searchS = () => {
  if (gradeClassId.value < 1 && courseId.value < 1) {
